...
public class PointAtTargetAndShootTrenchRun extends SequentialCommandGroup {
        ...
        private static final Pose2d initialPositionRight = new Pose2d(
                        Constants.fieldLength - Constants.initiationLine - 16,
        ...
        private static final Pose2d trenchStartCenter = new Pose2d(
                        Constants.fieldLength / 2 + Constants.trenchRunLength / 2,
                        powerCellLineY + 2, Rotation2d.fromDegrees(-170));
        private static final Pose2d trenchStartRight = new Pose2d(
                        Constants.fieldLength / 2 + Constants.trenchRunLength / 2,
        ...
        private static final Translation2d trenchVelocityConstraintTopRight = new Translation2d(
                        Constants.fieldLength / 2 + Constants.trenchRunLength / 2,
                        Constants.fieldWidth / -2 + Constants.trenchRunWidth);
        private static final RectangularRegionConstraint trenchVelocityConstraint = new RectangularRegionConstraint(
                        trenchVelocityConstraintBottomLeft, trenchVelocityConstraintTopRight,
                        new MaxVelocityConstraint(80));
        ...
        public PointAtTargetAndShootTrenchRun(boolean centerStart, DriveTrainBase driveTrain, RobotOdometry odometry,
                        LimelightInterface limelight, AHRS ahrs, Hopper hopper, ShooterRoller roller,
                                ...
                                new SequentialCommandGroup(
                                                centerStart ? new InstantCommand()
                                                                : new InstantCommand(() -> odometry
                                                                                .setPosition(initialPositionRight)),
                                                new ParallelCommandGroup(
                                                                new PointAtTargetWithOdometry(driveTrain, odometry,
                                                                                limelight),
                                                                new WaitCommand(1)
                                                                                .andThen(new WaitCommand(6)
                                                                                                .withInterrupt(() -> flywheel
                                                                                                                .atSetpoint())),
                                                                new WaitUntilCommand(() -> hood.atTargetPosition())),
                                                new ParallelRaceGroup(new RunHopper(hopper),
                                                                new RunShooterRoller(roller),
                                                                new HoldPosition(driveTrain),
                                                                new RunIntakeForwards(intake), new WaitCommand(1.5)))
                                                                                .deadlineWith(new RunShooterAtDistance(
                                                                                                flywheel, hood,
                                                                                                odometry, true)),
                                new SequentialCommandGroup(
                                                new TurnToAngleFast(driveTrain, ahrs, centerStart ? 135 : 175, true,
                                                                0.4),
                                                new InstantCommand(intake::extend),
                                                new NewRunMotionProfile(driveTrain, odometry,
                                                                List.of(centerStart ? trenchStartCenter
                                                                                : trenchStartRight, trenchEnd),
                                                                0, false, false,
                                                                List.of(trenchVelocityConstraint)).deadlineWith(
                                                                                new RunIntakeForwards(intake)),
                                                new InstantCommand(intake::retract),
                                                new NewRunMotionProfile(driveTrain, odometry, 0,
                                                                List.of(trenchEnd, secondShotPosition), 0,
                                                                true, false,
                                                                new ArrayList<>())
                                                                                .alongWith(new RunIntakeForwards(intake)
                                                                                                .withTimeout(0.5)),
