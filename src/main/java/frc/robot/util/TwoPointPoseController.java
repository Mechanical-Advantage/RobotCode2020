...
public class TwoPointPoseController {
  ...
  private final static double defaultConvergenceFactor = 0.5; // How quickly to converge behind the target
  private final static double defaultMaxLinearAngleRadians = 0.25 * Math.PI; // Max angle error at which linear velocity
                                                                             // is allowed
  ...
  /**
   * Creates a new TwoPointPoseController with default convergence factor and max
   * linear angle.
   * 
   * @param linearController        PID controller for correcting distance to
   *                                target
   ...
   */
  public TwoPointPoseController(PIDController linearController, double linearToleranceMeters,
      PIDController angularController, double angularToleranceRadians) {
    this(defaultConvergenceFactor, defaultMaxLinearAngleRadians, linearController, linearToleranceMeters,
        angularController, angularToleranceRadians);
  }

  /**
   * Creates a new TwoPointPoseController with custom convergence factor and max
   * linear angle.
   * 
   * @param convergenceFactor       (0-1) How quickly to converge behind the
   *                                target, lower values correct heading closer to
   *                                the target
   * @param maxLinearAngleRadians   Maximum allowable error on the angular
   *                                controller where linear velocity is used,
   *                                allows for turning in place
   * @param linearController        PID controller for correcting distance to
   *                                target
   * @param linearToleranceMeters   Acceptable target distance in meters
   * @param angularController       PID controller for correcting angle to target
   * @param angularToleranceRadians Acceptable angular error in radians
   */
  public TwoPointPoseController(double convergenceFactor, double maxLinearAngleRadians, PIDController linearController,
      double linearToleranceMeters, PIDController angularController, double angularToleranceRadians) {
  ...
  public ChassisSpeeds calculate(Pose2d currentPose) {
    ...
    Translation2d intermediateTarget = target
        .transformBy(new Transform2d(new Translation2d(xDist * convergenceFactor, 0), new Rotation2d()))
        .getTranslation();

    // Calculate relative target positions
    Translation2d relativeIntermediateTarget = intermediateTarget.minus(currentPose.getTranslation());
    Translation2d relativeTarget = target.getTranslation().minus(currentPose.getTranslation());

    // Run PID controllers
    output.vxMetersPerSecond = linearController.calculate(relativeTarget.getNorm() * (xDist > 0 ? 1 : -1));
    if (linearController.atSetpoint()) { // We're at the target, point in final direction
      ...
      angularController.setSetpoint(new Rotation2d(relativeIntermediateTarget.getX(), relativeIntermediateTarget.getY())
          .rotateBy(new Rotation2d(xDist > 0 ? Math.PI : 0)).getRadians());
    }
    output.omegaRadiansPerSecond = angularController.calculate(currentPose.getRotation().getRadians());
    if (angularController.atSetpoint()) {
      output.omegaRadiansPerSecond = 0;
    }

    // Decrease linear output when not facing correct direction
    double linearPower = 1 - Math.abs(angularController.getPositionError() / maxLinearAngleRadians);
